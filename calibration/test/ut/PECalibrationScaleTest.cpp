...
/**
 * Unit test of the PECalibrationScaleTest class.
 *
 * Code under test:
 *
 */
 ...
 * callback check
 */
TEST_F(PECalibrationScaleTest, ref_above_limit_in_between_CB)
{
   PE::Mock_normalisation norm_scale;

   PE::calibration_scale calib(norm_scale,10); //10 meters accuracy
   ...
   EXPECT_CALL(norm_scale,add_sensor(Eq(ByRef(ratio))));
 ...
 * callback check
 */
TEST_F(PECalibrationScaleTest, ignore_unused_ref_data_CB)
{
   PE::Mock_normalisation norm_scale;
